...
pub async fn add_comic_db (
    ...
    match auth_res{
        ...
        Err(_) => Err(SEVXError::AuthFailed(format!("Auth Failed of name:[{}] for Add Comic", auth.uname)))
    }
}
...
pub async fn update_comic_db (
    ...
    match auth_res{
        ...
        Err(_) => Err(SEVXError::AuthFailed(format!("Auth Failed of name:[{}] for Update Comic", auth.uname)))
    }
}
...
pub async fn delete_comic_db (
    ...
    // 先进行身份验证
    let auth = get_auth_db(&pool, delete_comic.name.clone(), delete_comic.password.clone()).await;
    match auth {
        Ok(_) => {
            // 判断当前 Comic 是否存在
        ...
        Err(_) => Err(SEVXError::AuthFailed(format!("Auth Failed of name:[{}] for delete Comic", delete_comic.name)))
    }
}
