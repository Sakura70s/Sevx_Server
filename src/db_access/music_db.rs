...
pub async fn delete_music_db (
    ...
    // 先进行身份验证
    let auth = get_auth_db(&pool, delete_music.name.clone(), delete_music.password.clone()).await;
    match auth {
        Ok(_) => {
            // 判断当前 Music 是否存在
        ...
        Err(_) => Err(SEVXError::AuthFailed(format!("Auth Failed of name:[{}] for delete Music", delete_music.name)))
    }
}
