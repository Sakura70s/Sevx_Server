...
pub async fn add_sv_db (
    ...
    match auth_res{
        ...
        Err(_) => Err(SEVXError::AuthFailed(format!("Auth Failed of name:[{}] for Add Sv", auth.uname)))
    }
}
...
pub async fn update_sv_db (
    ...
    match auth_res{
        ...
        Err(_) => Err(SEVXError::AuthFailed(format!("Auth Failed of name:[{}] for Update Sv", auth.uname)))
    }
}
